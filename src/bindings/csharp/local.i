...
// from UTF8 string (the callback function is used in libsbgn_wrap.cpp when 
...
// mapping for a type of function argument in libsbgn_wrap.cpp
%typemap(ctype,  out="unsigned int")        unsigned int        "long long"
%typemap(ctype,  out="unsigned int") const unsigned int& "const long long&"

// mapping for a type of function argument in csharp-files/libsbgnPINVOKE.cs
...
// conversion for a given argument in libsbgn_wrap.cpp
...
/**
* @return the most specific c# object possible for the given SbgnBase
* object.
*/
...
%{
  ...
  public static SbgnBase DowncastSbgnBase(IntPtr cPtr, bool owner)
  {
...
*/
%typemap("csout", excode=SWIGEXCODE) SbgnBase*
{
  SbgnBase ret = (SbgnBase) libsbgn.DowncastSbgnBase($imcall, $owner);$excode
  return ret;
}
...
*/
%typemap("csout", excode=SWIGEXCODE) SbgnTask*
{
  SbgnTask ret = (SbgnTask) libsbgn.DowncastSbgnBase($imcall, $owner);$excode
  return ret;
}
...
*/
%typemap("csout", excode=SWIGEXCODE) SbgnRange*
{
  SbgnRange ret = (SbgnRange) libsbgn.DowncastSbgnBase($imcall, $owner);$excode
  return ret;
}
...
* Convert Rule objects into the most specific object possible.
*/
%typemap("csout", excode=SWIGEXCODE) SbgnSimulation*
{
  SbgnSimulation ret = (SbgnSimulation) libsbgn.DowncastSbgnBase($imcall, $owner);$excode
  return ret;
}
...
*/
%typemap("csout", excode=SWIGEXCODE) SbgnOutput*
{
  SbgnOutput ret
  = (SbgnOutput) libsbgn.DowncastSbgnBase($imcall, $owner);$excode
  return ret;
}
...
* Convert SimpleSpeciesReference objects into the most specific object possible.
*/
%typemap("csout", excode=SWIGEXCODE) SbgnChange*
{
  SbgnChange ret
  = (SbgnChange) libsbgn.DowncastSbgnBase($imcall, $owner);$excode
  return ret;
}
...
* for C# proxy classes of SbgnBase subclasses and classes in libSEDML.
*
...
SWIGCS_EQUALS(SbgnBase)
SWIGCS_EQUALS(SbgnReader)
SWIGCS_EQUALS(SbgnWriter)
SWIGCS_EQUALS(ASTNode)
SWIGCS_EQUALS(XMLNamespaces)
SWIGCS_EQUALS(SbgnNamespaces)
...
* SbgnConstructorException (C# class) which is derived from
...
%typemap(csbase) SbgnConstructorException "System.ArgumentException";
%typemap(cscode) SbgnConstructorException 
%{
  internal SbgnConstructorException(IntPtr cPtr, bool cMemoryOwn, string v) : base(v)
  {
    swigCMemOwn = cMemoryOwn;
    swigCPtr    = new HandleRef(this, cPtr);
  }
  ...
  this(libsbgnPINVOKE.new_SbgnConstructorException__SWIG_0(), true, v) 
  {}
  %}
...
%{
  ...
  this(libsbgnPINVOKE.new_XMLConstructorException(), true, v) 
  {}
  %}
...
%pragma(csharp) imclasscode=%{
  class CustomExceptionHelper {
    ...
    static void SetPendingCustomException(string message, int type) {
      if (type == 0)
      SWIGPendingException.Set(new SbgnConstructorException(message));
      else 
...
*/
%typemap(csin) SbgnBase*       item "SbgnBase.getCPtrAndDisown($csinput)";
%typemap(csin) const SbgnBase* item "SbgnBase.getCPtr($csinput)";
...
*  - writeMathMLToString()
*
* Which take either an SbgnConstructorException or MathMLDocument as input.
*/
%typemap(csin) SbgnConstructorException   * "SbgnConstructorException.getCPtr($csinput)";
...
//  
//  1) SbgnReader::readSEDML(const std::string& filename)
//  2) readSEDML(const char* filename)
//  3) SbgnWriter::writeSEDML(SbgnConstructorException*, const std::string& filename)
//  4) writeSEDML(SbgnConstructorException*, const char* filename)
//
...
LIST_WRAPPER(SbgnNamespaces::getSupportedNamespaces,SbgnNamespaces)
