 ...
 * @brief   Contributed class that makes using libSBGN from python more convenient
 *          for the time being this only works for python 2.x, but won't break python 3
 * @author  Gordon Ball
 * 
 *<!---------------------------------------------------------------------------
 * This file is part of libSBGN.  Please visit http://sbgn.org for more
 * information about Sbgn, and the latest version of libSBGN.
 *
 ...
 * and also available online as http://sbgn.org/software/libsbgn/license.html
 *----------------------------------------------------------------------- -->*/

 
/**
 * Metaclass to convert getX/setX methods into python properties.
 * This is attached to SbgnBase and automatically applies for all classes
    ...
    def __new__(cls, classname, bases, classdict):
        ...
        for name in get_methods | set_methods:
            ...
            if name in get_methods:
                ...
                #test if function is "return _libsbgn.CLASS_getNAME(__args__)"
                if getter.func_code.co_names == ('_libsbgn', cname):
                    getter = getattr(_libsbgn, cname)
            ...
            if name in set_methods:
                ...
                if setter.func_code.co_names == ('_libsbgn', cname):
                    setter = getattr(_libsbgn, cname)
            ...
            if 'unset' + name in classdict:
                ...
                if deleter.func_code.co_names == ('_libsbgn', cname):
                    deleter = getattr(_libsbgn, cname)
